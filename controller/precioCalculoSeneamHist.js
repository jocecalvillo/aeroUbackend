...
const PrecioCalculoSeneam = require('../models/precioCalculoSeneamHist');
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioCalculoSeneam.find({ idHistorico: uid });
        ...
        const precioActualizado = await PrecioCalculoSeneam.findOneAndUpdateAndUpdate({ idHistorico: uid }, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioCalculoSeneam.findOneAndDelete({ idHistorico: uid });
