...
const PrecioSeguridad = require('../models/precioSeguridadHist');
...
const getPrecio = async (req, res = response) => {
    ...
    const [precios, total] = await Promise.all([
        ...
        PrecioSeguridad.countDocuments()
    ]);
...
const crearPrecio = async (req, res) => {
    ...
    try {
        ...
        const precio = new PrecioSeguridad(req.body);
        await precio.save();
        res.status(201).json({
            ok: true,
            precio
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioSeguridad.findById(uid);
        ...
        const precioActualizado = await PrecioSeguridad.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioSeguridad.findByIdAndDelete(uid);
