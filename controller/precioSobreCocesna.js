...
const PrecioSobreCocesna = require('../models/precioSobreCocesna');
...
const getPrecio = async (req, res = response) => {
    ...
    const [precios, total] = await Promise.all([
        ...
        PrecioSobreCocesna.countDocuments()
    ]);
...
const crearPrecio = async (req, res) => {
    ...
    try {
        ...
        const precio = new PrecioSobreCocesna(req.body);
        await precio.save();
        res.status(201).json({
            ok: true,
            precio
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioSobreCocesna.find({ idHistorico: uid });
        ...
        const precioActualizado = await PrecioSobreCocesna.findOneAndUpdateAndUpdate({ idHistorico: uid }, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioSobreCocesna.findOneAndDelete({ idHistorico: uid });
