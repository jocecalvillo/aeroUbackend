...
const PrecioHotelTripulacion = require('../models/precioHotelTripulacionHist');
...
const getPrecio = async (req, res = response) => {
    ...
    const [precios, total] = await Promise.all([
        PrecioHotelTripulacion
            .find({}, 'est importeUsd aeropuerto usuario')
            .skip(desde)
            .limit(10000000),

        PrecioHotelTripulacion.countDocuments()
    ]);
...
const crearPrecio = async (req, res) => {
    ...
    try {
        ...
        const precio = new PrecioHotelTripulacion(req.body);
        await precio.save();
        res.status(201).json({
            ok: true,
            precio
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear Precio Hotel'
        });
    }
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioHotelTripulacion.find({ idHistorico: uid });
        ...
        const precioActualizado = await PrecioHotelTripulacion.findOneAndUpdateAndUpdate({ idHistorico: uid }, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioHotelTripulacion.findOneAndDelete({ idHistorico: uid });
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al eliminar precio hotel'
        });
