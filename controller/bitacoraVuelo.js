...
const getXaefr = async (req, res = response) => {
    ...
    try {
        ...
        const [bitVuelos, total] = await Promise.all([
            XaEfr.find({}, 'logBook logBookVlo operacion leg fecha flt from to blkOut off onn blkIn blockHours flightHours flightCycles blkOutMex uplift depart arrive cargo cargoT cargoMvtT fuelVoucherLt fuelVoucherBitacoraTnX1000Lt fuelVoucherGal fuelVoucherBitacoraTnX1000Gal fuelVoucherTnTotal fuelVoucherGalTotal fuelVoucherLtTotal  errorBitacoraVsVoucher consumoLbX1000 observaciones fechaLocal start finish concepto')
                .skip(desde)
                .limit(100),
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al obtener xaEfr'
        });
    }

}

const crearXaefr = async (req, res = response) => {
    try {
        const xaEfr = new XaEfr(req.body);
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear xaEfr'
        });
    }
...
const getXaggl = async (req, res = response) => {
    ...
    const [bitVuelos, total] = await Promise.all([
        XaGgl.find({}, 'logBook logBookVlo operacion leg fecha flt from to blkOut off onn blkIn blockHours flightHours flightCycles blkOutMex uplift depart arrive cargo cargoT cargoMvtT fuelVoucherLt fuelVoucherBitacoraTnX1000Lt fuelVoucherGal fuelVoucherBitacoraTnX1000Gal fuelVoucherTnTotal fuelVoucherGalTotal fuelVoucherLtTotal  errorBitacoraVsVoucher consumoLbX1000 observaciones fechaLocal start finish concepto')
            .skip(desde)
            .limit(100),
...
const crearXaggl = async (req, res = response) => {
    try {
        const xaGgl = new XaGgl(req.body);
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear xaGgl'
        });
    }
...
const getXalfr = async (req, res = response) => {
    ...
    const [bitVuelos, total] = await Promise.all([
        XaLfr.find({}, 'logBook logBookVlo operacion leg fecha flt from to blkOut off onn blkIn blockHours flightHours flightCycles blkOutMex uplift depart arrive cargo cargoT cargoMvtT fuelVoucherLt fuelVoucherBitacoraTnX1000Lt fuelVoucherGal fuelVoucherBitacoraTnX1000Gal fuelVoucherTnTotal fuelVoucherGalTotal fuelVoucherLtTotal  errorBitacoraVsVoucher consumoLbX1000 observaciones fechaLocal start finish concepto')
            .skip(desde)
            .limit(100),
...
const crearXalfr = async (req, res = response) => {
    try {
        const xaLfr = new XaLfr(req.body);
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear xaLfr'
        });
    }
...
const getXalrc = async (req, res = response) => {
    ...
    const [bitVuelos, total] = await Promise.all([
        XaLrc.find({}, 'logBook logBookVlo operacion leg fecha flt from to blkOut off onn blkIn blockHours flightHours flightCycles blkOutMex uplift depart arrive cargo cargoT cargoMvtT fuelVoucherLt fuelVoucherBitacoraTnX1000Lt fuelVoucherGal fuelVoucherBitacoraTnX1000Gal fuelVoucherTnTotal fuelVoucherGalTotal fuelVoucherLtTotal  errorBitacoraVsVoucher consumoLbX1000 observaciones fechaLocal start finish concepto')
            .skip(desde)
            .limit(100),
...
const crearXalrc = async (req, res = response) => {
    try {
        const xaLrc = new XaLrc(req.body);
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear xaLrc'
        });
    }
...
const getXauyr = async (req, res = response) => {
    ...
    const [bitVuelos, total] = await Promise.all([
        XaUyr.find({}, 'logBook logBookVlo operacion leg fecha flt from to blkOut off onn blkIn blockHours flightHours flightCycles blkOutMex uplift depart arrive cargo cargoT cargoMvtT fuelVoucherLt fuelVoucherBitacoraTnX1000Lt fuelVoucherGal fuelVoucherBitacoraTnX1000Gal fuelVoucherTnTotal fuelVoucherGalTotal fuelVoucherLtTotal  errorBitacoraVsVoucher consumoLbX1000 observaciones fechaLocal start finish concepto')
            .skip(desde)
            .limit(100),
...
const crearXauyr = async (req, res = response) => {
    try {
        const xaUyr = new XaUyr(req.body);
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear xaUyr'
        });
    }
...
const leerExcel = async (req, res = response) => {
    try {
        ...
        await readExcel("F:\\vago\\back\\docs\\BitacoraVueloMayo2.xlsx");
