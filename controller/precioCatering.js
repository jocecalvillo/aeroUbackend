...
const PrecioCatering = require('../models/precioCatering');
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioCatering.findById(uid);
        ...
        const precioActualizado = await PrecioCatering.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioCatering.findByIdAndDelete(uid);
