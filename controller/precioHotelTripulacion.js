...
const PrecioHotelTripulacion = require('../models/precioHotelTripulacion');
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioHotelTripulacion.findById(uid);
        ...
        const precioActualizado = await PrecioHotelTripulacion.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioHotelTripulacion.findByIdAndDelete(uid);
