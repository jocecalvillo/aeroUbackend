...
const PrecioCalculoSeneam = require('../models/precioCalculoSeneam');
...
const getPrecio = async (req, res = response) => {
    ...
    const [precios, total] = await Promise.all([
        PrecioCalculoSeneam
            .find({}, 'distancia claveSenea kilometros tarifa importe usuario')
            .skip(desde)
            .limit(10000000),

        PrecioCalculoSeneam.countDocuments()
    ]);
...
const crearPrecio = async (req, res) => {
    ...
    try {
        ...
        const precio = new PrecioCalculoSeneam(req.body);
        await precio.save();
        res.status(201).json({
            ok: true,
            precio
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioCalculoSeneam.findById(uid);
        ...
        const precioActualizado = await PrecioCalculoSeneam.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioCalculoSeneam.findByIdAndDelete(uid);
