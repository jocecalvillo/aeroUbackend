...
const PrecioMotores = require('../models/precioMotores');
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioMotores.findById(uid);
        ...
        const precioActualizado = await PrecioMotores.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioMotores.findByIdAndDelete(uid);
