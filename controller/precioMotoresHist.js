...
const PrecioMotores = require('../models/precioMotoresHist');
...
const crearPrecio = async (req, res) => {
    ...
    try {
        ...
        const precio = new PrecioMotores(req.body);
        await precio.save();
        res.status(201).json({
            ok: true,
            precio
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear Precio Motores'
        });
    }
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioMotores.find({ idHistorico: uid });
        ...
        const precioActualizado = await PrecioMotores.findOneAndUpdateAndUpdate({ idHistorico: uid }, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioMotores.findOneAndDelete({ idHistorico: uid });
