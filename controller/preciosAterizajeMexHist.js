...
const PrecioAterrizajeMex = require('../models/precioAterrizajeMexHist');
...
const getPrecio = async (req, res = response) => {
    ...
    const [precios, total] = await Promise.all([
        PrecioAterrizajeMex
            .find({}, 'aterrizaje match nacInt horario precio usuario')
            .skip(desde)
            .limit(10000000),
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioAterrizajeMex.find({ idHistorico: uid });
        ...
        const precioActualizado = await PrecioAterrizajeMex.findOneAndUpdateAndUpdate({ idHistorico: uid }, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioAterrizajeMex.findOneAndDelete({ idHistorico: uid });
