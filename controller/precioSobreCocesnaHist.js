...
const PrecioSobreCocesna = require('../models/precioSobreCocesnaHist');
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioSobreCocesna.findById(uid);
        ...
        const precioActualizado = await PrecioSobreCocesna.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioSobreCocesna.findByIdAndDelete(uid);
