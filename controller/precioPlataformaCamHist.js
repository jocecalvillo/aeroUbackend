...
const PrecioPlataformaCam = require('../models/precioPlataformaCamHist');
...
const getPrecio = async (req, res = response) => {
    ...
    const [precios, total] = await Promise.all([
        ...
        PrecioPlataformaCam.countDocuments()
    ]);
...
const crearPrecio = async (req, res) => {
    ...
    try {
        ...
        const precio = new PrecioPlataformaCam(req.body);
        await precio.save();
        res.status(201).json({
            ok: true,
            precio
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioPlataformaCam.findById(uid);
        ...
        const precioActualizado = await PrecioPlataformaCam.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioPlataformaCam.findByIdAndDelete(uid);
