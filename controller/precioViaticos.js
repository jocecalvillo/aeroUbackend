...
const PrecioViaticos = require('../models/precioViaticos');
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioViaticos.findById(uid);
        ...
        const precioActualizado = await PrecioViaticos.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioViaticos.findByIdAndDelete(uid);
