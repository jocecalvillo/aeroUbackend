...
const PrecioRentaEquipo = require('../models/precioRentaEquipoRampaHist');
...
const crearPrecio = async (req, res) => {
    ...
    try {
        ...
        const precio = new PrecioRentaEquipo(req.body);
        await precio.save();
        res.status(201).json({
            ok: true,
            precio
        ...
        res.status(500).json({
            ok: false,
            msg: 'Error inesperado al crear Precio Renta Equipo'
        });
    }
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        const precioDB = await PrecioRentaEquipo.findById(uid);
        ...
        const precioActualizado = await PrecioRentaEquipo.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioRentaEquipo.findByIdAndDelete(uid);
