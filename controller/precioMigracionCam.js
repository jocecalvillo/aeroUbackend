...
const PrecioMigracionCam = require('../models/precioMigracionCam');
...
const actualizarPrecio = async (req, res = response) => {
    ...
    try {
        ...
        const precioActualizado = await PrecioMigracionCam.findByIdAndUpdate(uid, campos, { new: true });//new para regresar el actualizado
        res.status(200).json({
            ok: true,
            precioActualizado
        });
...
const borrarPrecios = async (req, res = response) => {
    ...
    try {
        ...
        await PrecioMigracionCam.findByIdAndDelete(uid);
